...
const Study = () => {
  ...
  const getTotalDuration = () => {
    if (mode === 'pomodoro') return (pomodoroPhaseRef.current === 'break' ? 5 : 25) * 60;
  ...
  const getDisplayTime = () => {
    if (mode === 'stopwatch') {
      ...
      if (mode === 'pomodoro' && pomodoroPhaseRef.current === 'work' && overtime > 0) {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {subjects.map((subjectItem) => (
                <motion.div
                  ...
                  onClick={() => navigate(`/study?subject=${encodeURIComponent(subjectItem.name)}`)}
                >
                  <div 
                  ...
                  <h3 className="text-xl font-bold text-white text-center mb-2">{subjectItem.name}</h3>
                ...
                <div className="flex flex-col items-center">
                  ...
                  {showCustomInput && (
                    <div className="w-full mb-8 p-6 bg-white/5 rounded-xl border border-[#6C5DD3]/30">
                      <label className="block text-white text-lg font-medium mb-3">Custom Duration (minutes)</label>
                      <div className="flex gap-3">
                        <input
                          type="number"
                          min="1"
                  ...
                  <div className="relative mb-12 w-[320px] h-[320px]">
                    {mode !== 'stopwatch' && (
                      <svg width="320" height="320" className="absolute inset-0">
                        <circle cx="160" cy="160" r="144" stroke="rgba(255,255,255,0.15)" strokeWidth="12" fill="none" />
                        <circle
                          cx="160" cy="160" r="144"
                          stroke={mode === 'pomodoro' && elapsedSeconds > getTotalDuration() && pomodoroPhaseRef.current === 'work' ? '#EF4444' : '#6C5DD3'}
                          strokeWidth="12"
                          fill="none"
                          strokeDasharray={2 * Math.PI * 144}
                          strokeDashoffset={2 * Math.PI * 144 * (1 - getProgress() / 100)}
                          style={{ transition: 'stroke-dashoffset 0.5s, stroke 0.2s' }}
                        />
                      </svg>
                    )}
                    <div className={`absolute inset-0 rounded-full bg-white/10 flex items-center justify-center ${mode === 'pomodoro' && elapsedSeconds > getTotalDuration() && pomodoroPhaseRef.current === 'work' ? 'text-red-400' : 'text-white'}`}>
                  ...
                  <div className="flex items-center gap-6">
                    ...
                    >
                      {isAmbientSoundOn ? <Volume2 className="w-5 h-5" /> : <VolumeX className="w-5 h-5" />}
                      Ambient Sound
                    </button>
                  </div>
                </div>
                    ...
                    <div className="relative mb-8 w-[160px] h-[160px]">
                      {mode !== 'stopwatch' && (
                        <svg width="160" height="160" className="absolute inset-0">
                          <circle cx="80" cy="80" r="64" stroke="rgba(255,255,255,0.15)" strokeWidth="8" fill="none" />
                          <circle
                            cx="80" cy="80" r="64"
                            stroke={mode === 'pomodoro' && elapsedSeconds > getTotalDuration() && pomodoroPhaseRef.current === 'work' ? '#EF4444' : '#6C5DD3'}
                            strokeWidth="8"
                            ...
                            strokeDashoffset={2 * Math.PI * 64 * (1 - getProgress() / 100)}
                            style={{ transition: 'stroke-dashoffset 0.5s, stroke 0.2s' }}
                          />
                        </svg>
                      )}
